...
from configs.front3d_config import Config as Front3d_Config
...
class FRONT3D_LDIF(Dataset):
    def __init__(self, config, mode):
        ...
        if mode == 'val':
            mode = 'test'
        split_file = os.path.join(config['data']['split'], mode , 'all_subset.json')
        with open(split_file) as file:
            samples = json.load(file)

        config_data = Front3d_Config('front3d')
        data_path = config_data.save_path
        sample_info = []
        skipped = 0
        for id, sample in tqdm(enumerate(samples)):
            # metadata = metadatas[id]
            imgid, objid, cname = sample
            img_name = f'{imgid.split("/")[-1].split(".")[0]}_{objid}'
            img_path = os.path.join(config_data.root_path, imgid)
            post_fix = img_path.split('.')[-1]
    
            info = {}
            rel_folder = os.path.join(data_path, img_name)
            info['img_path'] = os.path.join(rel_folder, img_name+'.npy')
            info['nss_points_path'] = os.path.join(rel_folder, 'nss_points.sdf')
            info['uniform_points_path'] = os.path.join(rel_folder, 'uniform_points.sdf')
            info['coarse_grid_path'] = os.path.join(rel_folder, 'coarse_grid.grd')
            ...
            info['sample_id'] = id
            info['class_id'] = config_data.classnames.index(cname)
            info['class_name'] = cname
            sample_info.append(info)
        print(f'{skipped}/{len(samples)} missing samples')
        self.split = sample_info
